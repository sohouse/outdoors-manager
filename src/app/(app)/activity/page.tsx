...
import ActivitySearchBar from "@/lib/features/activity/client/activity-search.tsx";
...
const ActivityPage: FC = () => {
    ...
    const [errorInfo, setErrorInfo] = useState<{ title: number; desc: string; stackTrace?: string } | null>(null);
    ...
    const fetchActivities = useCallback(async () => {
        try {
            ...
            const stackTrace = error instanceof ApplicationException && error.code === UNKNOWN_ERROR.code ? error.stack : '';
            setErrorInfo({ title: code, desc: message, stackTrace });
            throw error;
        } finally {
            setLoading(false);
        }
    ...
    return (
        <>
            {errorInfo && <ErrorAlert title={errorInfo.title} desc={errorInfo.desc} stackTrace={errorInfo.stackTrace} />}
            <div className="w-full flex flex-col gap-5 max-w-4xl mx-auto md:gap-5">
                <ActivitySearchBar />
                {isLoading && <div className="w-full space-y-4 p-4">
